...
You have entered a dungeon. You spot a small parcel that someone has left behind. Unwrapping the parcel, you find an anachronistic timepiece. The timepiece counts ticks, the standard unit of dungeon time. The tick counter starts at zero in each dungeon, and increments whenever you are about to walk onto another tile.

On closer inspection of the unfolded wrapping, you identify it as a magic map. The map shows a symbolic representation of your whereabouts. It also shows your position, counted in symbols from the start of the map, in left-to-right and top-to-bottom reading order. The first symbol has position zero. Taking the width of the dungeon into account, each position can also be decomposed into `(x, y)` coordinates, where `x` is the column and `y` is the row (both starting at zero). You study the map:
